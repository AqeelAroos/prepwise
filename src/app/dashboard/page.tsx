...
export default function Dashboard() {
  ...
  if (loading) return (
    <div style={{minHeight:'100vh', background:'#07090f', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'system-ui,sans-serif'}}>
      <div style={{textAlign:'center'}}>
        <div style={{width:'40px', height:'40px', border:'2px solid rgba(129,140,248,0.2)', borderTop:'2px solid #818cf8', borderRadius:'50%', animation:'spin 0.8s linear infinite', margin:'0 auto 16px'}}></div>
  ...
  return (
    <div style={{minHeight:'100vh', background:'#07090f', fontFamily:'system-ui,sans-serif', color:'white'}}>
      <style>{`
        @keyframes spin { to { transform: rotate(360deg) } }
        @keyframes fadeIn { from { opacity:0; transform:translateY(12px) } to { opacity:1; transform:translateY(0) } }
        .card-hover { transition: border-color 0.2s, transform 0.2s; }
        .card-hover:hover { border-color: rgba(129,140,248,0.2) !important; transform: translateY(-2px); }
        ...
        .nav-item:hover { background: rgba(129,140,248,0.06) !important; color: rgba(255,255,255,0.7) !important; }
      ...
      {/* Mobile top nav */}
      <div className="mobile-nav" style={{display:'none', justifyContent:'space-between', alignItems:'center', padding:'16px 20px', borderBottom:'1px solid rgba(255,255,255,0.06)', background:'#090c15', position:'sticky', top:0, zIndex:50}}>
        <span style={{fontWeight:800, fontSize:'18px'}}>CV<span style={{color:'#818cf8'}}>Prep</span></span>
        <div style={{display:'flex', gap:'10px', alignItems:'center'}}>
          <Link href="/analyze" style={{background:'linear-gradient(135deg,#6366f1,#818cf8)', color:'white', textDecoration:'none', padding:'8px 16px', borderRadius:'8px', fontWeight:700, fontSize:'13px'}}>
            + Analyze
          </Link>
      ...
      <div style={{display:'flex', minHeight:'100vh'}}>
        {/* Sidebar */}
        <aside className="sidebar" style={{width:'220px', background:'#090c15', borderRight:'1px solid rgba(255,255,255,0.05)', padding:'28px 0', display:'flex', flexDirection:'column', position:'fixed', top:0, bottom:0, left:0}}>
          ...
          <nav style={{padding:'20px 12px', flex:1}}>
            {[
              ...
              <Link key={item.label} href={item.href} className="nav-item" style={{display:'flex', alignItems:'center', gap:'10px', padding:'10px 12px', borderRadius:'8px', marginBottom:'4px', background:item.active ? 'rgba(129,140,248,0.1)' : 'transparent', color:item.active ? '#818cf8' : 'rgba(255,255,255,0.4)', textDecoration:'none', fontSize:'14px', fontWeight:item.active ? 600 : 400, transition:'background 0.2s, color 0.2s'}}>
        ...
        <main className="main-content" style={{marginLeft:'220px', flex:1, padding:'48px 52px', animation:'fadeIn 0.4s ease'}}>
          ...
          <div className="top-bar" style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'52px'}}>
            ...
            <Link href="/analyze" className="btn-hover" style={{display:'flex', alignItems:'center', gap:'8px', background:'linear-gradient(135deg,#6366f1,#818cf8)', color:'white', textDecoration:'none', padding:'12px 24px', borderRadius:'12px', fontWeight:700, fontSize:'14px', boxShadow:'0 4px 20px rgba(99,102,241,0.3)', whiteSpace:'nowrap'}}>
          ...
          <div className="stats-grid" style={{display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'16px', marginBottom:'48px'}}>
            {[
              {label:'Total Analyses', value:analyses.length, sub:analyses.length === 0 ? 'Start your first one' : `${analyses.length} jobs analyzed`, color:'#818cf8', icon:'◈'},
              {label:'Avg Match Score', value:avgScore !== null ? `${avgScore}%` : '—', sub:avgScore !== null ? (avgScore >= 70 ? 'Strong matches' : avgScore >= 50 ? 'Room to improve' : 'Needs work') : 'No data yet', color:avgScore !== null ? (avgScore >= 70 ? '#34d399' : avgScore >= 50 ? '#fbbf24' : '#f87171') : '#818cf8', icon:'◎'},
              {label:'This Month', value:analyses.filter(a => new Date(a.created_at) > new Date(Date.now() - 30*24*60*60*1000)).length, sub:'Last 30 days', color:'#c084fc', icon:'◷'},
            ].map(stat => (
              <div key={stat.label} className="card-hover" style={{background:'#0d1117', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'16px', padding:'22px'}}>
          ...
          <div>
            ...
            {analyses.length === 0 ? (
              <div style={{background:'#0d1117', border:'1px dashed rgba(255,255,255,0.08)', borderRadius:'16px', padding:'52px 24px', textAlign:'center'}}>
                <div style={{width:'48px', height:'48px', borderRadius:'12px', background:'rgba(129,140,248,0.1)', border:'1px solid rgba(129,140,248,0.2)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'22px', margin:'0 auto 16px'}}>✦</div>
                <div style={{color:'white', fontWeight:600, fontSize:'16px', marginBottom:'8px'}}>No analyses yet</div>
                <div style={{color:'rgba(255,255,255,0.3)', fontSize:'13px', marginBottom:'24px'}}>Analyze your first job application to get started</div>
                <Link href="/analyze" style={{background:'linear-gradient(135deg,#6366f1,#818cf8)', color:'white', textDecoration:'none', padding:'11px 24px', borderRadius:'10px', fontWeight:700, fontSize:'14px', boxShadow:'0 4px 16px rgba(99,102,241,0.25)'}}>
              ...
              <div style={{background:'#0d1117', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'16px', overflow:'hidden'}}>
                ...
                {analyses.map((a, i) => {
                  ...
                  const bg = sc >= 70 ? 'rgba(52,211,153,0.1)' : sc >= 50 ? 'rgba(251,191,36,0.1)' : 'rgba(248,113,113,0.1)'
                  return (
                    <div key={a.id} className="table-row row-hover" style={{display:'grid', gridTemplateColumns:'2fr 1.5fr 100px 120px', gap:'16px', padding:'14px 24px', borderBottom: i < analyses.length-1 ? '1px solid rgba(255,255,255,0.04)' : 'none', background:'transparent', transition:'background 0.15s'}}>
                      ...
                      <div className="table-company" style={{color:'rgba(255,255,255,0.4)', fontSize:'14px', display:'flex', alignItems:'center', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{a.company || '—'}</div>
