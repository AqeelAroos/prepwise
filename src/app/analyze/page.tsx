...
export default function Analyze() {
  ...
  const ta: React.CSSProperties = { width: '100%', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: 'white', borderRadius: '12px', padding: '14px 16px', fontSize: '14px', outline: 'none', resize: 'vertical', lineHeight: '1.6', fontFamily: 'system-ui,sans-serif', boxSizing: 'border-box' }
  const inp: React.CSSProperties = { width: '100%', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: 'white', borderRadius: '12px', padding: '12px 16px', fontSize: '14px', outline: 'none', fontFamily: 'system-ui,sans-serif', boxSizing: 'border-box' }
  const lbl: React.CSSProperties = { color: 'rgba(255,255,255,0.45)', fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', display: 'block', marginBottom: '8px' }

  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f', fontFamily: 'system-ui,sans-serif' }}>
      <style>{`
        @media (max-width: 768px) {
          .an-nav  { padding: 14px 20px !important; }
          .an-wrap { padding: 24px 16px !important; }
          .an-grid { grid-template-columns: 1fr !important; }
          .sc-card  { grid-template-columns: 1fr !important; }
          .sc-box   { min-width: unset !important; padding: 20px !important; }
          .st-grid  { grid-template-columns: 1fr !important; }
          .tab-bar  { overflow-x: auto; }
          .tab-btn  { padding: 12px 16px !important; font-size: 12px !important; }
          .act-btns { flex-direction: column !important; }
          .act-btns button, .act-btns a { width: 100% !important; text-align: center !important; justify-content: center !important; }
        }
      `}</style>

      {/* Nav */}
      <nav className="an-nav" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 48px', borderBottom: '1px solid rgba(255,255,255,0.06)', position: 'sticky', top: 0, background: 'rgba(10,10,15,0.95)', zIndex: 50, backdropFilter: 'blur(20px)' }}>
      ...
      <div className="an-wrap" style={{ maxWidth: '1100px', margin: '0 auto', padding: '48px 24px' }}>

        <div style={{ marginBottom: '32px' }}>
          <h1 style={{ color: 'white', fontSize: 'clamp(24px, 4vw, 34px)', fontWeight: 800, letterSpacing: '-1px', marginBottom: '6px' }}>New Analysis</h1>
        ...
        {!result ? (
          <div className="an-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
            ...
            <div style={{ background: '#111118', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '16px', padding: '20px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                <div>
                  ...
                  <input type="text" value={jobTitle} onChange={e => setJobTitle(e.target.value)} placeholder="e.g. Frontend Dev" style={inp} />
                </div>
                <div>
              ...
              <textarea value={jobDescription} onChange={e => setJobDescription(e.target.value)} placeholder="Paste the full job description here..." rows={13} style={ta} />
            </div>

            {/* Right */}
            <div style={{ background: '#111118', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '16px', padding: '20px' }}>
              <label style={lbl}>Your CV / Resume</label>
              <textarea value={cvText} onChange={e => setCvText(e.target.value)} placeholder="Paste your full CV text here..." rows={17} style={ta} />
            </div>

            {/* Button */}
            <div style={{ gridColumn: '1/-1' }}>
              {error && (
                <div style={{ background: 'rgba(220,38,38,0.1)', border: '1px solid rgba(220,38,38,0.2)', color: '#f87171', fontSize: '13px', padding: '12px 16px', borderRadius: '10px', marginBottom: '14px' }}>
                  {error}
                </div>
              )}
          ...
          <div>
            ...
            {/* Score card */}
            <div className="sc-card" style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '24px', marginBottom: '20px', background: '#111118', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '20px', padding: '28px', alignItems: 'center' }}>
              <div className="sc-box" style={{ background: scoreBg, border: `2px solid ${scoreColor}33`, borderRadius: '16px', padding: '24px 32px', textAlign: 'center', minWidth: '150px' }}>
              ...
              <div>
                ...
                <div style={{ color: 'white', fontWeight: 700, fontSize: 'clamp(16px,2.5vw,20px)', marginBottom: '10px' }}>
                  {jobTitle}{company ? ` at ${company}` : ''}
                </div>
                <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '14px', lineHeight: '1.7', marginBottom: '18px' }}>{result.summary}</p>

                {/* Action buttons */}
                <div className="act-btns" style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                  <button
                    onClick={() => { setResult(null); setCvText(''); setJobDescription(''); setJobTitle(''); setCompany(''); setTracked(false) }}
                  ...
                  <button
                    ...
                    style={{ background: tracked ? 'rgba(52,211,153,0.15)' : 'rgba(78,205,196,0.1)', border: `1px solid ${tracked ? 'rgba(52,211,153,0.3)' : 'rgba(78,205,196,0.25)'}`, color: tracked ? '#34d399' : '#4ecdc4', padding: '9px 18px', borderRadius: '8px', fontSize: '13px', cursor: tracked ? 'default' : 'pointer', fontWeight: 600, transition: 'all 0.2s' }}>
                    {tracked ? '✓ Added to Tracker' : tracking ? 'Adding...' : '+ Add to Tracker'}
                  </button>

                  <Link href="/tracker" style={{ background: 'rgba(129,140,248,0.1)', border: '1px solid rgba(129,140,248,0.25)', color: '#818cf8', padding: '9px 18px', borderRadius: '8px', fontSize: '13px', textDecoration: 'none', fontWeight: 600 }}>
                    View Tracker →
                  </Link>

                  <Link href="/dashboard" style={{ background: 'rgba(201,169,110,0.1)', border: '1px solid rgba(201,169,110,0.25)', color: '#c9a96e', padding: '9px 18px', borderRadius: '8px', fontSize: '13px', textDecoration: 'none' }}>
                    Dashboard →
                  </Link>
                </div>
              </div>
            ...
            {/* Strengths + Gaps */}
            <div className="st-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '18px' }}>
            ...
            <div style={{ background: '#111118', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '16px', overflow: 'hidden' }}>
              <div className="tab-bar" style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                {[
                  ...
                  <button key={tab.id} className="tab-btn" onClick={() => setActiveTab(tab.id)}
                    style={{ padding: '16px 24px', background: 'transparent', border: 'none', color: activeTab === tab.id ? '#c9a96e' : 'rgba(255,255,255,0.3)', fontSize: '14px', cursor: 'pointer', borderBottom: activeTab === tab.id ? '2px solid #c9a96e' : '2px solid transparent', fontWeight: activeTab === tab.id ? 600 : 400, whiteSpace: 'nowrap', transition: 'color 0.15s' }}>
                    {tab.label}
                  </button>
                ))}
              </div>
                ...
                {activeTab === 'cover' && (
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '14px' }}>
                      <button onClick={() => copyToClipboard(result.coverLetter)}
                        style={{ background: copied ? 'rgba(16,185,129,0.15)' : 'rgba(201,169,110,0.1)', border: `1px solid ${copied ? 'rgba(16,185,129,0.3)' : 'rgba(201,169,110,0.25)'}`, color: copied ? '#34d399' : '#c9a96e', padding: '8px 18px', borderRadius: '8px', fontSize: '13px', cursor: 'pointer' }}>
                        {copied ? '✓ Copied!' : '⎘ Copy'}
                      </button>
                    </div>
